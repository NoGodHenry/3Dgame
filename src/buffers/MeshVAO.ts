...
export class MeshVAO {
  ...
  public addData(mesh: Mesh): void {
    this.size = mesh.indicies.length;
    ...
    this.gl.bufferData(this.gl.ARRAY_BUFFER, new Float32Array(positionFloatBuffer), this.gl.STATIC_DRAW)
    
    this.indicies = this.gl.createBuffer();
    this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.indicies);
    this.gl.bufferData(this.gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(mesh.indicies), this.gl.STATIC_DRAW);
  }
